...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="drawer lg:drawer-open font-sans">
        ...
        <div className="navbar bg-base-100 border-b border-base-200 sticky top-0 z-30 px-4 lg:px-8">
          ...
          <div className="flex-none gap-4">
            ...
            <div className="dropdown dropdown-end">
              <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar border border-base-200 p-0 flex items-center justify-center overflow-hidden">
                <div className="w-10 h-10 rounded-full bg-base-200 flex items-center justify-center overflow-hidden">
                   {dbUser?.avatarUrl ? (
                     <img src={dbUser.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                   ) : (
                     <UserIcon className="w-5 h-5 text-base-content/60" />
                   )}
                </div>
              </div>
      ...
      <Sidebar session={session} userRole={userRole} dbUser={dbUser} />
    </div>
  )
}
