            ...
            <div className="grid gap-3">
                {displayTasks.length === 0 ? (
                    <div className="p-16 border-2 border-dashed border-base-200 rounded-3xl flex flex-col items-center justify-center opacity-40 text-center">
                        ...
                        <h4 className="text-sm font-black uppercase tracking-widest">No {activeTab} status detected</h4>
                        <p className="text-[10px] font-bold mt-1 max-w-[200px]">Strategic directives for this segment are currently null or pending initialization.</p>
                    </div>
                ) : (
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 mt-1.5 opacity-60">
                                        <div className="flex items-center gap-1.5">
                                            <div className="w-4 h-4 rounded-full bg-base-200 flex items-center justify-center text-[8px] font-black">
                                                {(task.assignee?.name || 'U').charAt(0)}
                                            </div>
                                            <span className="text-[10px] font-black uppercase tracking-tighter">{task.assignee?.name || 'Unassigned'}</span>
                                        </div>
                                        {task.sla && (
                                            <span className="text-[10px] font-bold px-2.5 py-1 bg-base-200/50 rounded-lg border border-base-300/50 flex items-center gap-1.5">
