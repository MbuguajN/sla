...
import { updateSubProject } from '@/app/actions/subProjectActions'
import { cn } from '@/lib/utils'

export default function SubProjectStatusManager({
    subProjectId,
    initialStatus,
    userDept,
    userRole
    ...
    // CS, BD, and Admin can manage subprojects
    const canManage = isAdmin || isCS || isBD
    ...
    async function handleStatusChange(newStatus: string) {
        ...
        setMessage(null)
        try {
            const res = await updateSubProject(subProjectId, { status: newStatus })
            if (res) {
                setStatus(newStatus)
    ...
    return (
        <div className="card bg-base-100 border border-base-200 shadow-sm p-5 space-y-1.5 relative group">
            <span className="text-xs font-black uppercase tracking-widest text-base-content/70">Lifecycle Phase</span>

            <div className="flex items-center justify-between">
                <div className={cn("flex items-center gap-2 font-black text-sm uppercase tracking-tighter", config.text)}>
                ...
                {canManage && (
                    <div className="dropdown dropdown-end">
                        ...
                        <ul tabIndex={0} className="dropdown-content z-[20] menu p-2 shadow-xl bg-base-100 rounded-2xl border border-base-200 w-48 mt-2 animate-in fade-in slide-in-from-top-2">
                            <li className="menu-title text-[10px] font-black uppercase tracking-widest opacity-40 px-3 py-1">Update Status</li>
            ...
            {message && (
                <p className={cn(
                    "text-[10px] font-black uppercase tracking-widest mt-1 animate-in fade-in",
