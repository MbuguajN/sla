...
interface Props {
  ...
  projectOptions: { id: number; title: string }[];
}
...
export default function RequisitionsClient({ initialRequisitions, projectOptions }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ title: "", departmentId: "", items: [{ name: "", quantity: 1, estimatedPrice: 0 }] });
      setCurrentStep(1);
      router.refresh();
    } catch (err) {
  ...
  const addItem = () => setFormData({ ...formData, items: [...formData.items, { name: "", quantity: 1, estimatedPrice: 0 }] });
  ...
  const nextStep = () => {
    if (currentStep === 1 && (!formData.title || !formData.departmentId)) return;
          ...
          <div key={item.id} className="group relative bg-white dark:bg-[#0f0f0f] border border-gray-100 dark:border-white/10 rounded-[2.5rem] p-1.5 transition-all duration-500 hover:shadow-2xl hover:shadow-amber-500/10 hover:-translate-y-2 overflow-hidden">
            <div className="bg-[#fcfdfe] dark:bg-black rounded-[2rem] p-6 space-y-6">
                ...
                <div className="pt-4 flex items-center justify-between border-t border-gray-100/50 dark:border-white/5">
                  <span className="text-[10px] font-black uppercase text-gray-400">GH₵ {item.totalAmount.toLocaleString()}</span>
                  <div className={cn(
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white dark:bg-black rounded-[2.5rem] shadow-2xl w-full max-w-2xl p-8 border border-gray-100 dark:border-white/10 overflow-hidden">
            ...
            <div className="min-h-[400px] flex flex-col justify-center">
               {currentStep === 1 && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                           <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Entity Source</span>
                           <select 
               ...
               {currentStep === 2 && (
                  <div className="space-y-4 animate-in fade-in slide-in-from-right-4 max-h-[400px] overflow-y-auto pr-2">
                     <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight mb-4">Material Manifest</h2>
                     {formData.items.map((item, idx) => (
                        <div key={idx} className="flex gap-4 p-4 bg-gray-50 dark:bg-white/5 rounded-2xl relative group">
                           <input 
                           ...
                           />
                           <input 
                              type="number" placeholder="Qty" value={item.quantity} onChange={e => updateItem(idx, "quantity", parseInt(e.target.value))}
                     ...
                     <button onClick={addItem} className="w-full py-4 border-2 border-dashed border-gray-100 dark:border-white/5 rounded-2xl font-black text-[10px] uppercase text-gray-400 hover:border-amber-600 transition-all">Add Material Segment</button>
                  </div>
               )}
               ...
               {currentStep === 3 && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 text-center">
                     ...
                     <p className="text-sm font-bold text-gray-400 max-w-xs mx-auto">Confirm your requisition for {formData.items.length} items. This will be transmitted to procurement.</p>
                  </div>
               )}
            </div>
            ...
            <div className="mt-10 flex items-center justify-between border-t border-gray-100 dark:border-white/5 pt-8">
               ...
               >
                  {loading ? "..." : currentStep === 3 ? "Transmit Request" : "Next Protocol"}
               </button>
            </div>
          </div>
        </div>
