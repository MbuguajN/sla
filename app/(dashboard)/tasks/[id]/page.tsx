    ...
    <div className="space-y-8 max-w-6xl mx-auto pb-32 px-4 lg:px-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pt-6 pb-4 border-b border-base-200/50">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm font-bold uppercase tracking-wider">
                ...
                {task.dueAt && (
                  <div className="flex items-center gap-1.5 px-2 py-1 bg-primary/10 rounded-lg text-primary">
                    ...
                    <span className="font-bold text-sm">Due: {format(task.dueAt, 'PPp')}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex flex-col lg:items-end gap-3 shrink-0 lg:ml-auto">
          <div className="flex items-center gap-3 bg-base-content/[0.03] px-4 py-3 rounded-xl border border-base-content/20">
            <span className="text-sm font-bold uppercase tracking-wider text-base-content/70">SLA</span>
            {task.dueAt && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          {/* Status Bar */}
          <div className="bg-base-100 p-2 rounded-xl border border-base-content/20 shadow-sm">
            <StatusControlBar
              taskId={task.id}
          ...
          {/* Description */}
          <div className="card bg-base-100 border border-base-content/20 shadow-sm rounded-2xl overflow-hidden">
            <div className="card-body p-6 gap-4">
              ...
              <div className="bg-base-content/[0.02] p-5 rounded-xl border border-base-content/20 text-sm text-base-content leading-relaxed">
          ...
          {/* Activity & Messages */}
          <div className="card bg-base-100 border border-base-content/20 shadow-sm overflow-hidden rounded-2xl">
            <div className="bg-base-content/[0.02] px-6 py-4 border-b border-base-content/20 flex items-center justify-between">
              <div className="flex items-center gap-2 text-primary">
                <MessageSquare className="w-4 h-4" />
                <h2 className="text-sm font-bold uppercase tracking-wider leading-none mb-0">Activity Log</h2>
              </div>
              <span className="badge badge-sm font-bold bg-base-content/5 text-base-content/70 border-none">{task.messages.length} Messages</span>
            </div>
            <div className="p-0">
              <TaskChat
                taskId={task.id}
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="card bg-base-100 border border-base-content/20 shadow-sm overflow-hidden sticky top-12 rounded-2xl">
            <div className="card-body p-0">
              {/* Assigned To */}
              <div className="p-5 space-y-3">
                <span className="text-sm font-bold uppercase tracking-wider text-base-content/30 flex items-center gap-1.5">
                  <User className="w-3.5 h-3.5 text-primary" /> Assigned To
                </span>
                <div className="flex items-center gap-3 bg-base-content/[0.02] p-3 rounded-xl border border-base-content/20">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-sm font-bold text-base-content/70 uppercase tracking-wider leading-none mt-0.5">Handler</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex flex-col gap-3 bg-base-content/[0.02] p-4 rounded-xl border border-base-content/20">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-bold text-base-content uppercase tracking-tight">{task.sla?.name}</span>
                    <span className="text-sm text-base-content/30 uppercase tracking-wider">Service Level Agreement</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="badge badge-sm bg-primary text-primary-content border-none font-bold text-sm uppercase tracking-wider h-6 px-3">
                      {task.sla?.durationHrs}H
                    </div>
                    <span className="text-sm font-bold text-base-content/70 uppercase tracking-wider">Time Limit</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {task.watchers.map((w: any) => (
                    <div key={w.id} className="tooltip" data-tip={w.user.name}>
                      <div className="avatar placeholder hover:scale-110 transition-transform">
                        <div className="bg-base-content/5 text-base-content rounded-full w-8 h-8 flex items-center justify-center border border-base-content/20 shadow-sm">
                          <span className="text-sm font-bold">{w.user.name?.charAt(0)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                  {task.watchers.length === 0 && (
                    <div className="w-full p-4 border border-dashed border-base-content/10 rounded-xl flex items-center justify-center">
                      <span className="text-sm text-base-content/70 italic text-center">No watchers</span>
                    </div>
                  )}
                </div>
              </div>
