        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="flex items-end justify-between border-b border-base-content/5 pb-6">
                ...
                <button onClick={() => setShowCreate(!showCreate)} className={cn(
                    "btn btn-sm gap-1.5 font-bold uppercase tracking-wider text-[10px]",
            ...
            {message && (
                <div className={cn("flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest py-2.5 px-4 rounded-xl border", message.type === 'success' ? 'bg-success/5 text-success border-success/20' : 'bg-error/5 text-error border-error/20')}>
            ...
            {showCreate && (
                <div className="bg-base-100 border border-base-content/10 rounded-2xl shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-1 duration-300">
                    {/* Form Header */}
                    <div className="px-6 py-4 border-b border-base-content/5 bg-base-200/30">
                        <h2 className="text-xs font-bold uppercase tracking-widest text-base-content/50 flex items-center gap-2">
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                <label className="text-[10px] font-bold uppercase tracking-widest text-base-content/25 block mb-1.5">Review Title</label>
                            ...
                            <div>
                                <label className="text-[10px] font-bold uppercase tracking-widest text-base-content/25 block mb-1.5">Deadline</label>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-[9px] text-base-content/20 mt-0.5">Questions managers use to rate their team members</p>
                                    </div>
                                    <button onClick={() => addQuestion('manager')} className="btn btn-ghost btn-xs text-primary gap-0.5 text-[10px]">
                                ...
                                <div className="space-y-2">
                                    {managerQuestions.map((q, i) => (
                                        <div key={i} className="flex items-center gap-2">
                                            <span className="text-[9px] font-bold text-base-content/15 w-5 shrink-0 text-center">{i + 1}.</span>
                                            ...
                                            {managerQuestions.length > 1 && (
                                                <button onClick={() => removeQuestion('manager', i)} className="btn btn-ghost btn-xs text-base-content/20 hover:text-error"><X className="w-3 h-3" /></button>
                                            )}
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-[9px] text-base-content/20 mt-0.5">Questions employees use to rate their manager</p>
                                    </div>
                                    <button onClick={() => addQuestion('employee')} className="btn btn-ghost btn-xs text-primary gap-0.5 text-[10px]">
                                        <Plus className="w-3 h-3" /> Add
                                    </button>
                                </div>
                                <div className="space-y-2">
                                    {employeeQuestions.map((q, i) => (
                                        <div key={i} className="flex items-center gap-2">
                                            ...
                                            {employeeQuestions.length > 1 && (
                                                <button onClick={() => removeQuestion('employee', i)} className="btn btn-ghost btn-xs text-base-content/20 hover:text-error"><X className="w-3 h-3" /></button>
                                            )}
                                        </div>
                                    ))}
                                </div>
                    ...
                    {/* Form Footer */}
                    <div className="px-6 py-4 border-t border-base-content/5 bg-base-200/20 flex items-center justify-between">
                        <p className="text-[9px] text-base-content/20">This will close any existing active cycle and notify all employees.</p>
                        <button onClick={handleCreate} disabled={creating || !title || !expiresAt} className="btn btn-primary btn-sm font-bold uppercase tracking-wider text-[10px] gap-1.5">
            ...
            <div className="space-y-3">
                {cycles.map((cycle: any) => (
                    <div key={cycle.id} className="bg-base-100 border border-base-content/10 rounded-2xl overflow-hidden shadow-sm">
                        <div className="px-6 py-4 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-base-content/25 flex items-center gap-1.5 mt-0.5">
                            ...
                            <div className="flex items-center gap-4">
                                {expandedResults === cycle.id && (
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="input input-xs input-bordered pl-8 w-40 text-[10px] font-bold"
                                            value={searchQuery}
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => handleViewResults(cycle.id)} className="btn btn-ghost btn-xs font-bold text-[10px] uppercase tracking-wider gap-1">
                                        Results {expandedResults === cycle.id ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                                    </button>
                                    {cycle.status === 'ACTIVE' && (
                                        <button onClick={() => handleClose(cycle.id)} className="btn btn-ghost btn-xs text-error font-bold text-[10px] uppercase tracking-wider gap-1">
                        ...
                        {expandedResults === cycle.id && results && (
                            <div className="border-t border-base-content/5 animate-in fade-in duration-200">
                                {results.length === 0 ? (
                                    <p className="text-center text-[11px] text-base-content/15 py-10 italic">No responses yet</p>
                                ) : (
                                            ...
                                            {results
                                                ...
                                                .map((r: any) => (
                                                    <tr key={r.id} className="hover:bg-base-content/[0.02]">
                                                        <td className="pl-6">
                                                            <div className="flex items-center gap-2">
                                                                <div className="w-7 h-7 rounded-lg bg-primary/10 text-primary flex items-center justify-center font-black text-[9px]">
                                                                    {r.name?.charAt(0) || '?'}
                                                                </div>
                                                                <span className="text-xs font-bold text-base-content">{r.name}</span>
                                                            </div>
                                                        </td>
                                                        <td><span className="text-[10px] text-base-content/30">{r.department?.name || r.role}</span></td>
                                                        <td className="max-w-[300px]">
                                                            <div className="flex flex-col gap-1 py-2">
                                                                {r.questionBreakdown.map((q: any) => (
                                                                    <div key={q.questionId} className="group/q">
                                                                        <p className="text-[9px] font-bold text-base-content/40 leading-tight">{q.questionText}</p>
                                                                        <div className="flex flex-wrap gap-1 mt-0.5">
                                                                            {q.ratings.map((rt: any, idx: number) => (
                                                                                <div key={idx} className="badge badge-xs bg-base-200 border-none text-[8px] flex gap-1 items-center px-1.5 py-2 group/tip relative">
                                                        ...
                                                        <td className="text-right pr-6">
                                                            <div className="flex items-center gap-1 justify-end">
                                                                ...
                                                                <span className="text-[9px] text-base-content/15">/10</span>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                ))}
                ...
                {cycles.length === 0 && (
                    <div className="text-center py-16">
                        <Star className="w-8 h-8 text-base-content/10 mx-auto mb-3" />
                        <p className="text-[11px] text-base-content/15 italic font-medium">No review cycles created yet</p>
                    </div>
                )}
            </div>
        </div>
