    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div className="flex items-end justify-between border-b border-base-content/20 pb-6">
                ...
                <div className="flex items-center gap-2 text-sm text-base-content/70 font-bold uppercase tracking-wider">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'Pending Requests', value: stats.pendingLeaves, sub: 'Awaiting review', color: stats.pendingLeaves > 0 ? 'text-error' : 'text-base-content/70' },
                ].map((kpi, i) => (
                    <div key={i} className="bg-base-100 border border-base-content/10 p-5 rounded-2xl space-y-1 shadow-sm">
                        <span className="text-xs font-black uppercase tracking-[0.25em] text-base-content/25 block">{kpi.label}</span>
                        <div className="flex items-baseline gap-2">
                            <span className={cn("text-2xl font-bold tracking-tight", kpi.color)}>{kpi.value}</span>
                            <span className="text-sm font-normal text-base-content/70 italic">{kpi.sub}</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <Link href="/hr/leaves" className="group bg-base-100 border border-base-content/10 rounded-2xl p-4 flex items-center gap-3 hover:border-warning/30 transition-all shadow-sm">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-base-content/25">{stats.pendingLeaves} pending</p>
                            </div>
                            <ArrowUpRight className="w-3.5 h-3.5 text-base-content/40 group-hover:text-warning transition-colors shrink-0" />
                        </Link>
                        ...
                        <Link href="/hr/reviews" className="group bg-base-100 border border-base-content/10 rounded-2xl p-4 flex items-center gap-3 hover:border-success/30 transition-all shadow-sm">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-base-content/25">{activeCycle ? 'Active cycle' : 'No active'}</p>
                            </div>
                            <ArrowUpRight className="w-3.5 h-3.5 text-base-content/40 group-hover:text-success transition-colors shrink-0" />
                        </Link>
                        ...
                        <Link href="/hr/suggestions" className="group bg-base-100 border border-base-content/10 rounded-2xl p-4 flex items-center gap-3 hover:border-info/30 transition-all shadow-sm">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-base-content/25">{stats.openSuggestions} open</p>
                            </div>
                            <ArrowUpRight className="w-3.5 h-3.5 text-base-content/40 group-hover:text-info transition-colors shrink-0" />
                        </Link>
                    </div>

                    {/* On Leave Table */}
                    <div className="bg-base-100 border border-base-content/10 rounded-2xl overflow-hidden shadow-sm">
                        <div className="px-6 py-4 border-b border-base-content/20 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <UserX className="w-4 h-4 text-warning" />
                                <h2 className="text-xs font-black uppercase tracking-widest text-base-content">Currently On Leave</h2>
                                <span className="text-xs bg-warning/10 text-warning font-bold px-2 py-0.5 rounded-lg">{onLeave.length}</span>
                            </div>
                            <Link href="/hr/leaves" className="text-sm font-bold text-primary uppercase tracking-wider hover:underline">
                                View All
                            </Link>
                        </div>
                        {onLeave.length === 0 ? (
                            <div className="text-center py-12 text-sm text-base-content/15 italic font-medium">No employees currently on leave</div>
                        ) : (
                            <div className="divide-y divide-base-content/5">
                                {onLeave.slice(0, 6).map((leave: any) => (
                                    <div key={leave.id} className="flex items-center gap-3 px-6 py-3 hover:bg-base-content/[0.02] transition-colors">
                                        <div className="w-8 h-8 rounded-lg bg-warning/10 text-warning flex items-center justify-center font-black text-sm">
                                            {leave.user.name?.charAt(0) || '?'}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-bold text-base-content truncate">{leave.user.name}</p>
                                            <p className="text-xs text-base-content/25">{leave.user.department?.name || 'N/A'}</p>
                                        </div>
                                        <span className="text-xs font-bold uppercase tracking-wider bg-warning/10 text-warning px-2 py-1 rounded-lg">{leave.type}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div className="bg-base-100 border border-base-content/10 rounded-2xl overflow-hidden shadow-sm">
                    <div className="px-6 py-4 border-b border-base-content/20 flex items-center gap-2">
                        <UserCheck className="w-4 h-4 text-success" />
                        <h2 className="text-xs font-black uppercase tracking-widest text-base-content">On Premise</h2>
                        <span className="text-xs bg-success/10 text-success font-bold px-2 py-0.5 rounded-lg">{onPremise.length}</span>
                    </div>
                        ...
                        <div className="divide-y divide-base-content/5">
                            {onPremise.map((user: any) => (
                                <div key={user.id} className="flex items-center gap-3 px-6 py-2.5 hover:bg-base-content/[0.02] transition-colors">
                                    <div className="w-7 h-7 rounded-lg bg-success/10 text-success flex items-center justify-center font-bold text-xs">
                                        {user.name?.charAt(0) || '?'}
                                    </div>
                                    <div className="min-w-0">
                                        <p className="text-sm font-bold text-base-content truncate">{user.name}</p>
                                        <p className="text-xs text-base-content/70 uppercase tracking-wider">{user.department?.name || user.role}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
